...
export function roosterAdapter(rooster: EduarteRooster, organisationId): Rooster {
  ...
  const afspraken: Rooster = rooster.items.map((item: Item) => { 
    ...
    return {
      ...
      startTime: item.beginDatumTijd,
      endTime: item.eindDatumTijd, startPeriod: item.beginLesuur,
      ...
      teachers: item.participanten
        ...
        .map((teacher) => {
          return {
            ...
            insertion: teacher.medewerker?.voorvoegsel
              ? teacher.medewerker?.voorvoegsel
              : undefined,
